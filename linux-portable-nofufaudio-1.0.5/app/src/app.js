...
function setupEventListeners() {
  ...
  audioPlayer.addEventListener('error', async () => {
    ...
    // Cancelación voluntaria → ignorar
    if (code === 1 /* MEDIA_ERR_ABORTED */) return;
    // Chromium puede disparar 'error' varias veces para el mismo fallo
    if (_audioRetryInProgress) return;

    const t = (currentIndex >= 0) ? queue[currentIndex] : null;
    if (!t) { hideLoading(); return; }

    // YouTube: re-resolver SIEMPRE (URL caducada, red, formato, etc.)
    if (t.type === 'youtube' && _audioRetryCount < 3) {
      ...
      t.url  = '';
      t.lazy = true;
      // En 2º reintento, forzar búsqueda de vídeo alternativo
...
async function playTrack(index) {
  ...
  audioPlayer.play()
    .then(async () => {
      ...
      updatePlaybackUI();
      hideLoading();
      // El AudioContext puede haberse suspendido en el mismo tick que play() resolvió
      // (race condition en Chromium/Electron). Forzar resume() tras la confirmación
      // de play garantiza que el grafo de audio esté activo y se oiga el sonido.
      if (_vizCtx && _vizCtx.state === 'suspended') {
        try { await _vizCtx.resume(); } catch(e) {}
      }
      // Si el vizCtx aún no existe en este punto (la primera vez se crea en el evento
      // 'play' que llega después de aquí), el watchdog se encargará de detectarlo.
    })
...
(function setupThemePanel() {
  ...
  function setVar(name, val){
    ...
    // Mantener settings.accentColor sincronizado para que applySettings no lo machaque
