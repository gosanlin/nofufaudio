...
function setupEventListeners() {
  ...
  audioPlayer.addEventListener('error', async () => {
    ...
    if (!t) return;
    if (t.type === 'youtube' && t.url && !t.lazy && _audioRetryCount < 2) {
      // La URL del stream puede haber caducado (~6h) o ser incompatible.
      // Marcamos como lazy para que playTrack vuelva a resolverla.
      _audioRetryCount++;
      console.warn(`[audio] Error en stream (intento ${_audioRetryCount}), re-resolviendo:`, t.title);
      t.url  = '';
      t.lazy = true;
      showToast('Reintentando stream…');
      // Limpiar src antes de reintentar para evitar disparar error otra vez
      audioPlayer.src = '';
      try { await playTrack(currentIndex); }
      catch(e) { hideLoading(); showToast('No se pudo reproducir: ' + t.title); }
    } else if (_audioRetryCount >= 2) {
      _audioRetryCount = 0;
      hideLoading();
      showToast('Error al reproducir: stream no disponible tras reintentos.');
    } else {
      hideLoading();
      showToast('Error al reproducir. Formato no soportado o archivo no encontrado.');
    }
  });
  // Resetear contador de reintentos cuando una pista se reproduce con éxito
  audioPlayer.addEventListener('playing', () => { _audioRetryCount = 0; });
...
function renderFavoritesUI() {
  ...
  // Update hero count and visibility
  const heroCount = document.getElementById('favorites-hero-count');
  if (heroCount) heroCount.textContent = `${favTracks.length} canción${favTracks.length !== 1 ? 'es' : ''}`;
  const favHero = document.getElementById('favorites-hero');
  if (favHero) favHero.style.display = favTracks.length ? '' : 'none';
...
(function initPlaylistDownload() {
  ...
  let dlDir = '';
  let _dlModalContext = 'playlist'; // 'playlist' | 'favorites'
  window._dlModalContext = _dlModalContext; // expose for favorites IIFE
  ...
  function openModal() {
    ...
    _dlModalContext = 'playlist';
    window._dlModalContext = 'playlist';
  ...
  window._dlModalClose = closeModal;
  window._dlModalDirRef = () => dlDir;
  window._dlModalSetDir = (d) => { dlDir = d; dirDisplay.textContent = d; };
  window._dlModalFormatVal = () => formatSel.value;
  ...
  confirmBtn.addEventListener('click', async () => {
    if (window._dlModalContext !== 'playlist') return;
    const pl = playlists.find(p => p.id === currentPlaylistId);
    if (!pl) return;
    const tracks = pl.tracks.map(id => findTrackById(id)).filter(Boolean);
...
/* ─────────────────────────────────────────────
   FAVORITOS — reproducir / descargar / borrar
───────────────────────────────────────────── */
(function initFavoritesActions() {
  try {
    ...
    if (playBtn) {
      playBtn.addEventListener('click', () => {
        const favTracks = favorites.map(id => findTrackById(id)).filter(Boolean);
        if (!favTracks.length) return showToast('No hay canciones en favoritos');
        queue = [...favTracks];
        updateQueueUI(); showView('queue'); playTrack(0);
      });
    }

    // ── Borrar todas las canciones de favoritos ──
    const clearBtn = document.getElementById('favorites-clear-btn');
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        if (!favorites.length) return showToast('Favoritos ya está vacío');
        if (!confirm(`¿Quitar las ${favorites.length} canciones de favoritos?`)) return;
        favorites = [];
        saveFavorites();
        renderFavoritesUI();
        renderLibraryUI();
    ...
    // ── Descargar todos los favoritos (reutiliza el modal de descarga de playlist) ──
    ...
    if (dlBtn && backdrop) {
      dlBtn.addEventListener('click', () => {
        const favTracks = favorites.map(id => findTrackById(id)).filter(Boolean);
        if (!favTracks.length) return showToast('No hay canciones en favoritos');
        window._dlModalContext = 'favorites';
        if (window._dlModalSetDir) window._dlModalSetDir(settings.downloadDir || '');
        if (formatSel) formatSel.value = settings.downloadFormat || 'mp3';
        if (infoDiv) infoDiv.textContent = `Se descargarán ${favTracks.length} canción${favTracks.length !== 1 ? 'es' : ''} en la carpeta elegida.`;
        backdrop.style.display = 'flex';
      });

      if (confirmBtn) {
        confirmBtn.addEventListener('click', async () => {
          if (window._dlModalContext !== 'favorites') return;
          const favTracks = favorites.map(id => findTrackById(id)).filter(Boolean);
          if (!favTracks.length) { showToast('No hay canciones en favoritos'); if (window._dlModalClose) window._dlModalClose(); return; }

          let dir = window._dlModalDirRef ? window._dlModalDirRef() : '';
          if (!dir) {
            try { dir = await window.nofuf.downloadDefaultFolder(); }
            catch (e) { showToast('No se pudo determinar la carpeta de descarga'); return; }
          }
          ...
          settings.downloadDir    = dir;
          settings.downloadFormat = formatSel ? formatSel.value : 'mp3';
          saveSettings();

          if (window._dlModalClose) window._dlModalClose();
          openDownloadsPanel();
          showToast(`Descargando ${favTracks.length} canción${favTracks.length !== 1 ? 'es' : ''}…`);

          const fmt = (formatSel ? formatSel.value : null) || 'mp3';
          const batchSize = 3;
          for (let i = 0; i < favTracks.length; i += batchSize) {
            const batch = favTracks.slice(i, i + batchSize);
            await Promise.all(batch.map(async track => {
              const id = 'dl-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
              addDownloadRow(id, track);
              try {
  ...
  } catch(e) { console.error('[initFavoritesActions]', e); }
})();
...
(function setupThemePanel() {
  ...
  if (bh) bh.addEventListener('input', () => {
    // CSS usa --bar (no --bar-h) para la altura del reproductor
